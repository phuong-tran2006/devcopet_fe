...
const QuizPage = () => {
  ...
  useEffect(() => {
    ...
    if (lessonId) {
      Promise.all([
        courseApi.getLessonQuiz(lessonId).catch(() => ({ questions: [] })),
        courseApi.getLessonDetail(lessonId).catch(() => null),
      ])
        ...
        {!submitted && questions.length > 0 && (
          <div className="mt-10 flex justify-end">
            <button
              ...
              className="bg-primary-fixed-dim text-on-primary-fixed font-bold px-10 py-3.5 rounded-xl hover:bg-primary-fixed hover:scale-105 active:scale-95 transition-all shadow-[0_0_15px_rgba(0,128,128,0.4)] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 disabled:shadow-none"
            >
              Submit Answers
            </button>
          </div>
