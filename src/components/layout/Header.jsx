      ...
      <nav className="flex justify-between items-center px-margin-desktop h-20 w-full">
        ...
        <div className="flex items-center gap-6">
          {isAuthenticated ? (
              ...
              <div className="relative group cursor-pointer ml-2 z-50">
                ...
                <div className="absolute top-10 right-0 w-40 bg-surface-container-high/95 backdrop-blur-lg border border-white/10 rounded-xl shadow-[0_10px_30px_rgba(0,0,0,0.8)] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 overflow-hidden transform origin-top-right group-hover:scale-100 scale-95">
                  <div className="flex flex-col gap-1 p-2">
                    ...
                    <button onClick={handleLogout} className="w-full text-left px-3 py-2 text-sm text-error hover:bg-error/10 rounded-lg transition-colors font-medium flex items-center gap-2">
            ...
            <>
              <Link to="/login" className="font-body-md text-body-md text-on-surface-variant hover:text-on-surface transition-all duration-300 ease-out-cubic active:scale-95">
                Login
              </Link>
